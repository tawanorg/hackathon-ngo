...
const Recycle: NextPage = () => {
  ...
  const [products, setProducts] = useState([])
   ...
   const withFilter = (p) => {
     if (isSubmit) {
       return p.id === isSubmit?.id
     }
     return p
   }
   console.log('submittedDonation', submittedDonation, selectedProduct)
  return (
    <div className="d-flex flex-col">
      <QuickView selectedProduct={selectedProduct}>
         
         {submittedDonation?.id === selectedProduct?.id ? (
           <div className="d-flex flex-row justify-center w-full py-4">
             <h3 className="text-lg mb-6">Select an item you like to recycle</h3>
            <ul role="list" className="-my-6 divide-y divide-gray-200 mb-8">
              {products.filter(withFilter).map((product) => (
                <div key={product.id} 
                className={classNames(
                  selectItem?.id === product?.id ? 'bg-yellow-200' : null,
                  'flex py-2  hover:bg-yellow-200 rounded mb-2'
                )}
              ...
              <div className="shadow overflow-hidden sm:rounded-md">
                ...
                {isSubmit?.id === selectedProduct?.id ? (
                  <span>Your details has been sent to the recycle organisation.<br /> Someone will get back  to you</span>
                ) : (
                  <button
                    type="submit"
...
export default withPageAdmin(Recycle, {
  page: {
    title: 'Recycle',
