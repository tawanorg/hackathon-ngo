...
export default function QuickView({ isOpen = false, selectedProduct, product = DEFAULT_PRODUCT }: Props) {
  const [open, setOpen] = useState(isOpen)
  const [selectedColor, setSelectedColor] = useState(product.metadata?.colors[0])
  const [selectedSize, setSelectedSize] = useState(product.metadata?.rams[2])
              ...
              <div className="w-full relative flex items-center bg-white px-4 pt-14 pb-8 overflow-hidden shadow-2xl sm:px-6 sm:pt-8 md:p-6 lg:p-8">
                ...
                <div className="w-full grid grid-cols-1 gap-y-8 gap-x-6 items-start sm:grid-cols-12 lg:gap-x-8">
                  <div className="aspect-w-2 aspect-h-3 rounded-lg bg-gray-100 overflow-hidden sm:col-span-4 lg:col-span-5">
                    <img src={product.imageSrc} alt="" className="object-center object-cover" />
                  </div>
